...
import os
from model_updata.SQL_data_kwass import dic_merge
import json

class GenerateQueModel(object):
    def __init__(self,modelpath):
    
        self.model_save_path=''
        self.opt = Options().parse()
        
        if not os.path.exists(self.opt.modelpath_kw[1]):
            os.mkdir(self.opt.modelpath_kw[1])
        if not os.path.exists(self.opt.modelpath_kw[0]):
            os.mkdir(self.opt.modelpath_kw[0])
            
        if modelpath=='modelA':
            self.model_save_path =  self.opt.modelpath_kw[0]
        elif modelpath=='modelB':
            self.model_save_path =  self.opt.modelpath_kw[1]
    ...
    def load_basicinfo(self):
        ques_dic_ori= dic_merge()
        
        #print(len(ques_list))
        #for ques in ques_list:
        #    ques_list_tmp = line.strip('\n')
        iter=0
        ques_dic={}
        for ques,ids in ques_dic_ori.items():
            ques_cut=self._cut_doc(Converter('zh-hans').convert(ques))
            self.ques_list.append(ques_cut)
            self.documents[iter]=ques_cut
            ques_dic[iter]=[ids,ques]
            iter+=1
        #print(self.documents)
        with open(self.model_save_path+self.opt.quesdic, 'w', encoding='utf-8') as jsonf:
            js=json.dumps(ques_dic)
            jsonf.write(js)
